...
| [3654. Minimum Sum After Divisible Sum Deletions](https://leetcode.com/problems/minimum-sum-after-divisible-sum-deletions/description/) | `Modulo`, `Prefix Sum` | This is a **linear DP with prefix-sum state compression**. Let `prefix[i]` be the sum of the first `i` elements, and define `dp[i]` as the minimum possible remaining sum after processing the first `i` elements. Normally, we can keep `nums[i-1]`, giving `dp[i] = dp[i-1] + nums[i-1]`. If a contiguous subarray `j..i-1` is deleted and its sum is divisible by `k`, then `prefix[i] % k == prefix[j] % k`, and we can jump directly from state `j` to `i` with zero cost: `dp[i] = min(dp[i], dp[j])`. To avoid O(n²) enumeration of all `j`, we maintain an auxiliary array `minDP[r]`, which stores the minimum `dp[j]` among all previous positions with `prefix[j] % k == r`. Then the transition becomes `dp[i] = min(dp[i-1] + nums[i-1], minDP[prefix[i] % k])`. Each state is computed in O(1), resulting in an overall O(n) time complexity. This problem is a good example where deletions appear to be “forward decisions”, but the optimal transition depends only on the best historical state grouped by prefix-sum modulo. | ⭐️⭐️⭐️⭐️⭐️ |
...
| [3704. Count No-Zero Pairs That Sum to N](https://leetcode.com/problems/count-no-zero-pairs-that-sum-to-n/description/)  | `LSD to MSD` | Key trick: low digit → high digit with borrow/carry. The only cross-digit dependency is borrow (or carry). At digit i, after applying borrow, pick digits a_i and b_i such that (a_i + b_i + carry) mod 10 = n_i, then propagate nextBorrow/nextCarry to digit i+1.  Leading zeros only via endedA/endedB (not started). In low→high order, “leading zeros” mean: once a digit becomes 0, all higher digits must stay 0; otherwise 0 would be an internal digit (invalid). Use flags endedA, endedB: if endedA=1 then a_i must be 0; update endedA' = endedA OR (a_i = 0) (same for B).  Ones digit rule: a_0 and b_0 cannot be 0 (trailing zero is not a leading zero).  State: dp[i][borrow][endedA][endedB] (memoized DFS). Base case: valid iff borrow = 0 after the highest digit.  |⭐️⭐️⭐️⭐️⭐️⭐️⭐️|
